...
{
    printf("FIND CHAR AND DESR INDEX\n");
    uint8_t error = 0xff;
...
void uart_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (xQueueReceive(spp_uart_queue, (void * )&event, (portTickType)portMAX_DELAY)) {
            printf("UART TASK INIT: after queue receive\n");
            switch (event.type) {
            //Event of UART receving data
            case UART_DATA:
                printf("CASE UART_DATA\n");
                if ((event.size)&&(is_connected)) {
                    printf("CASE UART_DATA: if is connected\n");
                    uint8_t * temp = NULL;
...
static void spp_uart_init(void)
{
    ...
    printf("SPP UART TASK INIT: after task create\n");
}
...
void spp_heartbeat_task(void * arg)
{
    ...
    printf("HEARTBEAT QUEUE RECEIVE\n");            
            while(1){
                heartbeat_count_num++;
...
void spp_cmd_task(void * arg)
{
    ...
    for(;;){
        ...
        if(xQueueReceive(cmd_cmd_queue, &cmd_id, portMAX_DELAY)) {
            printf("SPP CMD TASK: after queue receive\n");
            esp_log_buffer_char(GATTS_TABLE_TAG,(char *)(cmd_id),strlen((char *)cmd_id));
            printf("%s\n", (char *)cmd_id);     // AQUI AQUI É O DO WRITE ABF3
            free(cmd_id);
        }
    }
    vTaskDelete(NULL);
...
static void spp_task_init(void)
{
    ...
    printf("SPP TASK INIT: after task create\n");
}

static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    printf("GAP EVENT HANDLER\n");    
    esp_err_t err;
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    printf("GATTS PROFILE EVENT HANDLER\n");
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    // uart_write_bytes(UART_NUM_0, "yo\n", p_data->write.len);
                    printf("%s\n", (char *)(p_data->write.value));  // AQUI AQUI AQUI AQUI É O PRIMEIRO WRITE TO ABF1
#endif
                }else{
                    printf("TALVEZ ESTE?\n");
                    //TODO:
